...
export const getUserBasedId = asyncError(async (req, res, next) => {
  console.log;

  const userById = await UserCollection.findById(req.params.userId);

  if (!userById) return next(new ErrorHandler("User Not Found", 404));
...
export const resetPassword = asyncError(async (req, res, next) => {
  ...
  if (!password) return next(new ErrorHandler("Incorect Password", 400));
...
export const follow = asyncError(async (req, res, next) => {
  res.status(200).json({
    success: true,
    message: "You Have Follow",
  });
});

export const unFollow = asyncError(async (req, res, next) => {
  res.status(200).json({
    success: true,
